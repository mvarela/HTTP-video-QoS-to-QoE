...
colnames(dfrm) <- c("id", "lr","mlbs","bandwidth","segment_length","N","IV","sd","q10th","q90th","se","ci")
...
form.in <- as.formula('IV~lr+mlbs+bandwidth+segment_length')
...
colnames(dfrm.test) <- c("id", "lr","mlbs","bandwidth","segment_length","N","IV","sd","q10th","q90th","se","ci")

lr <- dfrm.test$lr/max(dfrm.test$lr)
mlbs <- dfrm.test$mlbs/max(dfrm.test$mlbs)
bw <- dfrm.test$bandwidth/max(dfrm.test$bandwidth)
sl <- dfrm.test$segment_length/max(dfrm.test$segment_length)

net.results <- compute(mod, cbind(lr,mlbs,bw,sl))
